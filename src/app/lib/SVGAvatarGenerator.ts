...
export class SVGAvatarGenerator {
  ...
  private selectRandomVibe(input: string): string {
    // Get all vibe names for deterministic selection
    const vibeNames = Object.keys(VIBES);
    const hash = this.hashInput(input);
    const index = hash % vibeNames.length;
    return vibeNames[index];
  }
  ...
  public generate(
    ...
    // Priority: custom colors > hex-based palette > vibe colors > random vibe selection
    let colors: string[];
    if (customColors && customColors.length > 0) {
      ...
      // If no vibe is provided, select one deterministically based on input
      const selectedVibe = vibe || this.selectRandomVibe(input);
      colors = this.getVibeColors(selectedVibe);
    }
    ...
    const gradientDefs = gradients
      .map(
        (grad) => `
      <radialGradient id="${grad.id}" cx="${grad.cx}%" cy="${grad.cy}%" r="${
          grad.r
        }%">
        <stop offset="0%" stop-color="${grad.color1}" stop-opacity="${
          grad.opacity
        }" />
        <stop offset="30%" stop-color="${grad.blendedColor}" stop-opacity="${
          grad.opacity * 0.8
        }" />
        <stop offset="60%" stop-color="${grad.color2}" stop-opacity="${
          grad.opacity * 0.4
        }" />
    ...
    const gradientFills = gradients
      .map((grad, index) => {
        const blendMode =
          index === 0 ? "normal" : index % 2 === 1 ? "multiply" : "overlay";
  ...
  <defs>
    <radialGradient id="base" cx="50%" cy="50%" r="70%">
      ...
      <stop offset="50%" stop-color="${
        shuffledColors[1] || shuffledColors[0]
      }" stop-opacity="0.6" />
      <stop offset="100%" stop-color="${
        shuffledColors[2] || shuffledColors[0]
      }" stop-opacity="0.4" />
    </radialGradient>
    ${gradientDefs}
  </defs>
